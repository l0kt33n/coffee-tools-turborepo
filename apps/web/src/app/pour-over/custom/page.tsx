            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <div className="rounded-md border p-4 bg-slate-50 dark:bg-slate-900">
                              ...
                              <div className="space-y-2 text-sm">
                                <div className="p-2 border rounded-md bg-white dark:bg-slate-800">
                                  ...
                                  <div>
                                    Pour {bloomWater}g of water (
                                    {waterTemperature || 0}°{temperatureUnit})
                                    to bloom the coffee grounds
                                  </div>
                                ...
                                {Array.from({ length: pours }).map((_, i) => (
                                  <div
                                    key={i}
                                    className="p-2 border rounded-md bg-white dark:bg-slate-800"
                                  >
                                    <div className="font-semibold">
                                      Step {i + 2}
                                    </div>
                                    <div>
                                      Pour {waterPerPour}g of water (Total:{" "}
                                      {bloomWater + waterPerPour * (i + 1)}g)
                                    </div>
                                    <div className="text-slate-500">
                                      Wait{" "}
                                      {(() => {
                                        try {
                                          ...
                                          return Math.max(
                                            1,
                                            Math.floor(remainingTime / pours),
                                          );
                                        } catch (error) {
                                          console.error(
                                            "Error calculating pour time:",
                                            error,
                                          );
                                ...
                                <div className="p-2 border rounded-md bg-white dark:bg-slate-800">
                                  ...
                                  <div>
                                    Allow water to drain through the coffee bed
                                  </div>
                                </div>
                              </div>
                            </div>
                                ...
                                {fields.map((field, index) => {
                                  ...
                                  if (
                                    ...
                                    if (!advancedSteps[0].isBloom) {
                                      ...
                                      newSteps[0] = {
                                        waterAmount:
                                          advancedSteps[0].waterAmount || 0,
                                        duration:
                                          advancedSteps[0].duration || 0,
                                        isBloom: true,
                                        description:
                                          advancedSteps[0].description,
                                      };
                                  ...
                                  return (
                                    <div
                                      key={field.id}
                                      className="flex flex-col gap-2 p-3 border rounded-md bg-slate-50 dark:bg-slate-900"
                                    >
                                      <div className="flex items-end gap-2">
                                        <div className="flex-1">
                                          <FormLabel
                                            className={`text-xs ${index === 0 ? "text-blue-600" : ""}`}
                                          >
                                            {index === 0
                                              ? "Bloom Water (g)"
                                              : "Water (g)"}
              ...
              <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    Coffee: <span className="font-bold">{totalCoffee}g</span>
                  </div>
                  <div>
                    Water: <span className="font-bold">{totalWater}g</span>
                  </div>
                  <div>
                    Ratio: <span className="font-bold">1:{ratio}</span>
                  </div>
                  <div>
                    Temperature:{" "}
                    <span className="font-bold">
                      {waterTemperature || 0}°{temperatureUnit} ({otherUnitTemp}
                      °{temperatureUnit === "C" ? "F" : "C"})
                    </span>
                  </div>
                  ...
                  {mode === "basic" ? (
                    <>
                      <div>
                        Pours: <span className="font-bold">{pours}</span>
                      </div>
                      <div>
                        Bloom Water:{" "}
                        <span className="font-bold">{bloomWater}g</span>
                      </div>
                      <div>
                        Water per Pour:{" "}
                        <span className="font-bold">{waterPerPour}g</span>
                      </div>
                      <div>
                        Total Brew Time:{" "}
                        <span className="font-bold">
                    ...
                    <>
                      ...
                      <div>
                        Advanced Total:{" "}
                        <span className="font-bold">{advancedTotalWater}g</span>
                      </div>
                      <div
                        className={`col-span-2 ${waterDifference === 0 ? "text-green-600" : "text-amber-600"}`}
                      >
                        {Math.abs(waterDifference) > 0 ? (
                          <>
                            {waterDifference > 0 ? "Remaining: " : "Excess: "}
                            <span className="font-bold">
                              {Math.abs(waterDifference)}g
                            </span>
                          </>
                        ) : (
