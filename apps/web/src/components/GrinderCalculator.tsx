...
export function GrinderCalculator() {
  ...
  useEffect(() => {
    const loadGrinders = async () => {
      try {
        ...
        if (
          savedGrinder &&
          data.grinders.some((g: Grinder) => g.name === savedGrinder)
        ) {
  ...
  const filteredGrinders = useMemo(() => {
    ...
    return fuse
      .search(searchQuery)
      .map((result: FuseResult<Grinder>) => result.item);
          ...
          <CardContent>
            <div className="grid gap-4">
              <div>
                ...
                <p className="text-xl">
                  {formatSetting(
                    result.calculated_setting,
                    result.setting_format,
                  )}
                </p>
              </div>
                    ...
                    {result.matching_methods.map((method, idx) => (
                      <li key={idx} className="border-b pb-2 last:border-0">
                        ...
                        <div className="text-sm text-muted-foreground">
                          Range: {method.start_microns} - {method.end_microns}{" "}
                          microns ({method.grind_category})
                        </div>
