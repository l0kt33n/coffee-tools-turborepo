...
const oneZpressoGrinders = grinderData.grinders.filter((g) =>
  g.name.toLowerCase().includes("1zpresso"),
) as Grinder[];
if (oneZpressoGrinders.length === 0)
  throw new Error("No 1Zpresso grinders found in grinderData.json");
  ...
  describe("complex settings", () => {
    ...
    oneZpressoGrinders.forEach((grinder) => {
      describe(`1Zpresso ${grinder.name} (${grinder.clicks_per_number} clicks per number)`, () => {
        ...
        const v60Method = grinder.brew_methods.find(
          (m) => m.method_name === "V60",
        );
        if (!v60Method) {
        ...
        if (
          !v60Method.start_setting ||
          !v60Method.end_setting ||
          v60Method.start_microns === null ||
          v60Method.end_microns === null
        ) {
          console.warn(
            `V60 method settings or microns are null for ${grinder.name}`,
          );
          return;
        }
        ...
        it("should correctly map middle microns", () => {
          ...
          if (result && typeof result === "string") {
            const [startRotations, startNumbers, startClicks] = startSetting
              .split(".")
              .map(Number);
            const [endRotations, endNumbers, endClicks] = endSetting
              .split(".")
              .map(Number);
            const [resultRotations, resultNumbers, resultClicks] = result
              .split(".")
              .map(Number);

            // Check if result is between start and end settings
            const startValue =
              (startRotations ?? 0) * 100 +
              (startNumbers ?? 0) * 10 +
              (startClicks ?? 0);
            const endValue =
              (endRotations ?? 0) * 100 +
