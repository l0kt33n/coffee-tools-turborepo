...
export const formatSetting = (
  ...
  if (format === "simple") {
    ...
    if (parts.length === 3) {
      ...
      const rotations = parseInt(parts[0] ?? "0");
      const numbers = parseInt(parts[1] ?? "0");
...
export const mapMicronsToSetting = (
  ...
  if (settingFormat === "simple") {
    ...
    const minSettingNum =
      typeof minSetting === "string" ? parseInt(minSetting) : minSetting;
    const maxSettingNum =
      typeof maxSetting === "string" ? parseInt(maxSetting) : maxSetting;
    ...
    const parseComplexSetting = (
      ...
      if (parts.length === 3) {
        ...
        const numValue =
          rotations * 10 * clicksPerNumber + numbers * clicksPerNumber + clicks;
...
export const STANDARD_BREW_METHODS: Record<
  string,
  {
    name: string;
    min_microns: number;
 ...
 */
export const queryMicrons = async (
  params: QueryParams,
): Promise<QueryResult> => {
  ...
  let matchingMethods = grinder.brew_methods.filter((method) => {
    ...
    if (
      brewMethod &&
      !method_name.toLowerCase().includes(brewMethod.toLowerCase())
    ) {
      return false;
    }
  ...
  if (matchingMethods.length > 0) {
    ...
    if (method) {
      if (
        method.start_setting &&
        method.end_setting &&
        method.start_microns !== null &&
        method.end_microns !== null
      ) {
    ...
    for (const method of grinder.brew_methods) {
      ...
      if (setting_format === "simple") {
        try {
          const startVal =
            typeof start_setting === "string"
              ? parseInt(start_setting)
              : start_setting;
          const endVal =
            typeof end_setting === "string"
              ? parseInt(end_setting)
              : end_setting;
  ...
  matchingMethods = matchingMethods
    .map((method) => {
      ...
      if (
        methodName.includes("v60") &&
        targetMicrons >= 450 &&
        targetMicrons <= 550
      ) {
        fitScore -= 0.3; // Better fit
      } else if (
        methodName.includes("espresso") &&
        targetMicrons >= 200 &&
        targetMicrons <= 300
      ) {
        fitScore -= 0.3;
