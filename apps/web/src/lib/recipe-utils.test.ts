...
describe("Recipe Utilities", () => {
  ...
  describe("createCustomRecipe", () => {
    it("creates a recipe with correct values based on input parameters", () => {
      ...
      // Check bloom step
      const bloomStep = recipe.steps.find((step) => step.type === "bloom");
      const pourStep = recipe.steps.find((step) => step.type === "pour");
      const drawdownStep = recipe.steps.find(
        (step) => step.type === "drawdown",
      );

      if (!bloomStep || !pourStep || !drawdownStep) {
        throw new Error("Required steps not found");
      }
    ...
    it("uses default bloomMultiplier when not provided", () => {
      ...
      if (!bloomStep) {
        throw new Error("Bloom step not found");
      }
